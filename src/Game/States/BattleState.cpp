...
void BattleState::tick()
{
    ...
    if (canInteract)
    {
        ...
        else if (currentEnemyHealth <= 0 && phase == 2 && enemy->getIsBoss() == 1)
        {
            setNextState("Win");
            setFinished(true);
            return;
        }
        else if (currentEnemyHealth <= 0 && enemy->getIsBoss() == 1)
        {
            currentEnemyHealth = enemy->getMaxHealth();
            phase++;
        } 
        else if (currentEnemyHealth <= 0 && enemy->getIsBoss() == 0) 
        {
            setNextState("Win");
            setFinished(true);
            return;
...
void BattleState::render()
{
    ...
    ofDrawBitmapString(enemy->getIsBoss(), 100, 100);
    ofDrawBitmapString(currentEnemyHealth, 110, 100);
